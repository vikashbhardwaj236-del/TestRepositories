...
public class D2hActivationdata_NewDB {

	public static ExcelUtilities Exls = new ExcelUtilities(
			"D:\\Dishtv_ActivationData_Prepration\\D2h_Installation_Activation_Data.xlsx");
	static String Installation_sheet = "Install_data";
	static String Activation_sheet = "Activation_data";
	static String strloader = "//*[@id='imgAJAXLoader']";
	static Select sel;

	String Namevalue = "Test ";
	//String StrCurrentDT = ApplicationUtilities.Fun_DateTime();
	String kittyy;
	static public String DTserverNode1 = "d2hUAT-N1.dishtv-india.com,21343,1433";
	static public String DTserverNode11 = "DEV-DISH-V1-N11.DishtvIndia-New.in,21453";
	static public String DTserverNode2 = "d2hUAT-N2.dishtv-india.com,21344";
	static public String DTserverNode3 = "d2hUAT-N3.dishtv-india.com,21345";
	static public String DTDBuserID = "Gen_Testing";
	static public String DTDBPass = "gentesting@12345";
	static public String DT_India_DB = "d2h2019";
	static public String DT_India_SR_DB = "d2h_SRCRM_2019";
	static public String DT_SMSDTH_DB = "smSd2h2019";
	...
	public static void deletefile() {
		try {
			File folder = new File("D:\\Dishtv_ActivationData_Prepration");
	...
	public static void createfile() {
		try {
			...
			FileWriter myWriter = new FileWriter("D:\\Dishtv_ActivationData_Prepration\\ProcessDone " + StrDt + ".txt");
	...
	public static LinkedList<String> update_On_schemeId() {
		...
		try {
			...
			DatabaseConnection Objcon1 = new DatabaseConnection(DTserverNode1, DTDBuserID, DTDBPass, DT_SMSDTH_DB);
			...
			for (int i = 2; i <= runstatus + 1; i++) {
				...
				for (String itreatedata : complainttrndata) {
					...
					Objcon.executeupdate("update d2h2019.dbo.Voucherinfo set schemeid = " + SchemeId
							+ ", usedforActivation = 'N', LocationID =" + Location_ID+ " where voucherno in ('" + Voucher + "')");
					Objcon1.executeupdate("update  SMSd2h2019.dbo.Subscribermaster set Hwvoucherno = " + SchemeId
							+ " where Hwvoucherno  in ('" + Voucher + "')");
					Objcon1.executeupdate("update SMSd2h2019.dbo.Itemserialnomaster_voucher set schemeid =" + SchemeId
	...
	public static CopyOnWriteArrayList<String> validateComplaiantTRN_OnNode1() {
		...
		try {
			...
			while (getvoucher.hasNext()) {
				...
				DatabaseConnection Objcon = new DatabaseConnection(DTserverNode1, DTDBuserID, DTDBPass, DT_SMSDTH_DB);
	...
	public static CopyOnWriteArrayList<String> getVoucher_ScratchCode() {
		...
		try {
			 ...
			 * "  VoucherCode,VoucherNo   from d2h2019.dbo.Voucherinfo (nolock) where  Activationid is null and locationtype ='DL' and SchemeId = 717 and VoucherCode is not null  order by Date_time desc"
			 * );
			 */

			ResultSet result1 = Objcon.executeQuery("Select top " + randomno
					+ "  VoucherCode,VoucherNo   from d2h2019.dbo.Voucherinfo (nolock) \r\n"
					+ "			where  Activationid is null and locationtype ='DL' \r\n"
					+ "			and SchemeId = 1658 and VoucherCode is not null and voucherno not in \r\n"
					+ "			(Select distinct  top 10 Voucherno from d2h_SRCRM_2019.dbo.Complainttrn (nolock) where  voucherno is not null and voucherno <>'' and complaintdt>getdate()-30) \r\n"
	...
	public static LinkedList<String> updateActivation() {
		...
		try {
			...
			for (int i = 2; i <= count + 1; i++) {
				String scheme = Exls.getCellData(Activation_sheet, 7, i);
				String ModelRowId = Exls.getCellData(Activation_sheet, 8, i);
				String locationId = Exls.getCellData(Activation_sheet, 9, i);
				for (String getdata : data) {
					String[] VC_STB = ApplicationUtilities.split(getdata, ":");
					String VCNo = VC_STB[0].toString();
					String STBNo = VC_STB[1].toString();
					/*Objcon.executeupdate("update d2h2019.dbo.Stbinfo set Schemeid = " + scheme
							+ ",LocationType='DC',LocationId= " + locationId + ",stbmodelrowid =" + ModelRowId
							+ " where stbno in ('" + VCNo + "'" + "," + "'" + STBNo + "'" + ")");
*/
					/*Objcon.executeupdate("update d2h2019.dbo.vcinfo set Schemeid = " + scheme
							+ ",LocationType='DC',LocationId= " + locationId + ",stbmodelrowid =" + ModelRowId
							+ " where VCNo in ('" + VCNo + "'" + "," + "'" + STBNo + "'" + ")");*/

					Objcon.executedelete("delete from d2h2019.dbo.libertyboxdump where serialnumber in('" + VCNo + "'"
							+ "," + "'" + STBNo + "'" + ")");
	...
	public static CopyOnWriteArrayList<String> GetdataActivation() {
		...
		try {
			...
			ResultSet result = Objcon.executeQuery("Select top " + count
					+ " * from d2h2019.dbo.Stbinfo a (nolock) where LocationType = 'SP' and stbmodelrowid in (6701,6702,6704)  and VSC_ID is not null and SwapBoxflag<>1\r\n" + 
					"and not exists (select stbno from d2h2019.dbo.subscribers(nolock) b where a.stbno=b.stbno)");
