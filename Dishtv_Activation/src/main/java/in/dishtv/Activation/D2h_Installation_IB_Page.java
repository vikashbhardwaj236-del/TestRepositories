 ...
 *
 */
public class D2h_Installation_IB_Page extends BaseLibrary {
	...
	public D2h_Installation_IB_Page() {
		super();
	...
	public void IB_Pageopen() {
		try {
			//*[@id="ctl00_RadMenu1"]/ul/li[2]/a/span
	...
	public void Move_To_Installation_Page(int k) {
		try {
			...
			/*if (strMSG != "::: Installation Request :::") {
				for (String winHandle : driver.getWindowHandles()) {
					//Thread.sleep(5000);
					driver.switchTo().window(winHandle);
					strMSG = driver.switchTo().window(winHandle).getTitle();
		...
		System.out.println("a123");
			...
			for (String winHandle : driver.getWindowHandles()) {
				...
				if (strMSG.contains(":: Request ::")) {
					...
					Thread.sleep(2000);
					scallcategory = driver.findElement(By.xpath("//*[@id=\"txtComplaintCategory\"]"))
							.getAttribute("value");
					Thread.sleep(2000);
					...
					ltoken = Integer.parseInt(strMSG.replaceAll("[\\D]", ""));
					strtoken = Long.toString(ltoken);
					xls.setCellData(Installation_sheet, "Ticket no", k, strtoken);
			...
			xls.setCellData(Installation_sheet, "Issues/Remarks", k, "Issue in Token generation");
			System.out.println("Exception in Fillcomplaint" + ex);
		}
	}
