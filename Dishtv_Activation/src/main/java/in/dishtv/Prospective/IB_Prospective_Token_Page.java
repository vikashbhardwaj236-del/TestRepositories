 ...
 *//**
	 ...
	 *
	 *//*
		 * public class IB_Prospective_Token_Page extends BaseLibrary { String StrPath =
		 * System.getProperty("user.dir") +
		 * "//testdata//DishD2h_Phoenix_Prospective.xlsx"; ExcelUtilities xls = new
		 * ExcelUtilities(StrPath); String SheetName = "Dish_Prospective"; public String
		 ...
		 * complalclose;
		 * 
		 * public IB_Prospective_Token_Page() { super();
		 ...
		 * driver.findElement(By.id("ddlRegionalLang")).click(); Thread.sleep(2000); new
		 * Select(driver.findElement(By.id("ddlRegionalLang"))).selectByVisibleText(
		 * "HINDI"); Thread.sleep(5000); driver.findElement(By.id("btnSave")).click();
		 * WebDriverWait wait = new WebDriverWait(driver, 15);
		 * wait.until(ExpectedConditions.alertIsPresent()); // WebDriverWait wait = new
		 ...
		 * System.out.println(strtoken); ltoken =
		 * Integer.parseInt(strtoken.replaceAll("[\\D]", "")); strtoken =
		 * Long.toString(ltoken); xls.setCellData(SheetName, "Token No", i, strtoken);
		 * driver.switchTo().defaultContent(); Fill_Complaint_Details(i);
		 ...
		 * (Leadtype.equalsIgnoreCase("SMRT Stick")) {
		 * driver.findElement(By.id("rdolistLeadType_2")).click(); } else if
		 * (Leadtype.equalsIgnoreCase("SMRT Stick + Alexa")) {
		 ...
		 * }
		 * 
		 * public void Fill_Complaint_Details(int k) { try { String winHandleBefore =
		 * driver.getWindowHandle(); String strMSG = ""; // Switch to new window opened
		 * for (String winHandle : driver.getWindowHandles()) {
		 * driver.switchTo().window(winHandle); strMSG =
		 * driver.switchTo().window(winHandle).getTitle(); Thread.sleep(2000);
		 * Thread.sleep(2000); if (strMSG.contains(":: Request ::")) { //
		 * complalclose.click(); Thread.sleep(2000); strMSG =
		 * ApplicationUtilities.Dynamic_MobileNo(); complmobile.sendKeys(strMSG);
		 * Thread.sleep(2000); strMSG = ApplicationUtilities.Dynamic_MobileNo();
		 * complaltmobile.sendKeys(strMSG); Thread.sleep(2000); scallcategory =
		 ...
		 * Thread.sleep(4000); driver.findElement(By.id("btnSave")).click();
		 * Thread.sleep(4000); assertEquals(closeAlertAndGetItsText(),
